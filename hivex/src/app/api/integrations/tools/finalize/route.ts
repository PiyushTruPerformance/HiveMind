...
import { SUPPORTED_NANGO_PROVIDERS, finalizeConnection } from '@/lib/server/integrations/nango'

/** POST /api/integrations/tools/finalize { provider } — record the Nango connection for the caller. */
...
export function POST(request: Request) {
  return handle(async () => {
    ...
    return finalizeConnection({ provider, workspaceId: workspace.id, userProfileId: ctx.profile.id })
  })
}
